        ...
        return `<div class="pending-item">
          <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0">
            ...
            <div>
              ...
              <div style="font-size:11px;color:var(--text3);margin-top:2px">${timeAgo(c.creato_il)}</div>
            </div>
          </div>
...
export async function sendChallenge(preselectedId) {
  const targetId = preselectedId || document.getElementById('challenge_target')?.value;
  ...
  await addFeedEvent({
    tipo: 'challenge_sent', player1_id: state.currentUser.id, player2_id: targetId,
    metadata: { messaggio: msg }
  });

  await sendPushNotification({
    playerIds: [targetId], title: 'Nuova Sfida',
    body: `${state.currentUser.nome} ti ha sfidato${msg ? ': "' + msg + '"' : '!'}`,
    tag: 'challenge', url: '/#sfide'
  });
...
export async function acceptChallenge(challengeId) {
  ...
  await patch('challenges', `id=eq.${challengeId}`, {
    stato: 'accepted', risposto_il: new Date().toISOString()
  });

  await addFeedEvent({
    tipo: 'challenge_accepted', player1_id: state.currentUser.id, player2_id: c.sfidante_id,
    metadata: {}
  });

  await sendPushNotification({
    playerIds: [c.sfidante_id], title: '✅ Sfida Accettata!',
    body: `${state.currentUser.nome} ha accettato la tua sfida. Preparati!`,
    tag: 'challenge-accepted', url: '/#sfide'
  });
...
export async function refuseChallenge(challengeId) {
  ...
  await patch('challenges', `id=eq.${challengeId}`, {
    stato: 'refused', risposto_il: new Date().toISOString()
  });

  await addFeedEvent({
    tipo: 'challenge_refused', player1_id: state.currentUser.id, player2_id: c.sfidante_id,
    metadata: {}
  });

  await sendPushNotification({
    playerIds: [c.sfidante_id], title: '😤 Sfida Rifiutata',
    body: `${state.currentUser.nome} ha rifiutato la tua sfida.`,
    tag: 'challenge-refused', url: '/#sfide'
  });
